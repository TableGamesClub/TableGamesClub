	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			if (type.equalsIgnoreCase("MEMBER")) {// 讀取member表格
				...
				pstmt = conn
						.prepareStatement("select imgFileName, storeImage from StoreMember where storeMemberId = ?");
				pstmt.setInt(1, storeMemberId);
			} else if(type.equalsIgnoreCase("GroupRoom")){// 讀取GroupRoom表格
				// 讀取瀏覽器傳送來的帳號(storeMemberId)
				Integer groupSerialNumber = new Integer(request.getParameter("id"));
				pstmt = conn
						.prepareStatement("select imgFileName, privateGroupImage from GroupRoom where groupSerialNumber = ?");
				pstmt.setInt(1, groupSerialNumber);
			}
			if (type != null) {
