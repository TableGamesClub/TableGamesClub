...
@WebServlet(
		name="map2",
		urlPatterns={"/FindStoreByMapServlet"}
		)
public class FindStoreByMapServlet extends HttpServlet{
	GroupService service = null;
	StoreMemberService service2 = null;
	GetXY convert = null;
	GroupRoomDAO_Interface grdao = null;
	StoreInformationDAO_Interface sdao = null;
	public FindStoreByMapServlet() {
		service = new GroupService();
		service2 = new StoreMemberService();
		convert = new GetXY();
...
//			Map<String,List<GroupRoom>> StoreMap = new HashMap<String, List<GroupRoom>>();
			
			ApplicationContext context = new ClassPathXmlApplicationContext(
					"model-config1-DriverManagerDataSource.xml");
			StoreInformationDAO_Interface sidao = (StoreInformationDAO_Interface) context
					.getBean("StoreInformationDAO");
			List<StoreInformation> storeList = sidao.getAll();
			for(StoreInformation bean2 : storeList){
...
//				json.put("EndTime", EndTime);
				...
				json.put("StoreTel", bean2.getStoreTel());
				json.put("RentAreaCost", bean2.getRentAreaCost());
				json.put("StoreAddress", bean2.getStoreAddress());
...
//		response.sendRedirect("findjodmap.jsp");
		RequestDispatcher rd = request
				.getRequestDispatcher("/StoreInfoMap.jsp");
